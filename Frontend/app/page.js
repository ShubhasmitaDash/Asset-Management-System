...
function App() {
  ...
  const handleCustomSelectChange = (e) => {
    ...
    if (value === 'custom') {
      ...
      if (name === 'category') setIsCustomCategory(true)
      if (name === 'vendor') setIsCustomVendor(true)
      return
    }

    setForm((prev) => ({ ...prev, [name]: value }))
    if (name === 'category') setIsCustomCategory(false)
    if (name === 'vendor') setIsCustomVendor(false)
  }

  const handleDateSelect = (name, date) => {
    if (name === 'purchaseDate' && date && date < startOfToday) return
    setForm((prev) => ({ ...prev, [name]: date ? format(date, 'yyyy-MM-dd') : '' }))
    if (name === 'purchaseDate') setPurchaseDateOpen(false)
    if (name === 'warrantyExpiry') setWarrantyDateOpen(false)
  }
  ...
  const handleSubmit = (e) => {
    ...
    const purchaseDateValue = form.purchaseDate ? new Date(`${form.purchaseDate}T00:00:00`) : null
    if (purchaseDateValue && purchaseDateValue < startOfToday) {
                          ...
                          <div>
                            ...
                            <Popover open={purchaseDateOpen} onOpenChange={setPurchaseDateOpen}>
                              <PopoverTrigger asChild>
                                <button
                                  type="button"
                                  className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm text-left flex items-center justify-between gap-2 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-700"
                                >
                                  <span className={form.purchaseDate ? 'text-gray-700' : 'text-gray-400'}>
                                    {form.purchaseDate ? format(new Date(form.purchaseDate), 'MMM d, yyyy') : 'Select date'}
                                  </span>
                              ...
                              <PopoverContent className="w-auto p-0" align="start">
                                <Calendar
                                  ...
                                  onSelect={(date) => handleDateSelect('purchaseDate', date)}
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              <div className="relative">
                                <select
                                  name="vendor"
                                  value={isCustomVendor ? 'custom' : form.vendor}
                                  onChange={handleCustomSelectChange}
                                  className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm bg-white appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-700"
                                >
                            ...
                            <Popover open={warrantyDateOpen} onOpenChange={setWarrantyDateOpen}>
                              <PopoverTrigger asChild>
                                <button
                                  ...
                                  className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm text-left flex items-center justify-between gap-2 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-700"
                                >
                                  <span className={form.warrantyExpiry ? 'text-gray-700' : 'text-gray-400'}>
                                    {form.warrantyExpiry ? format(new Date(form.warrantyExpiry), 'MMM d, yyyy') : 'Select date'}
                                  </span>
                                  <CalendarDays className="w-4 h-4 text-gray-500" />
                                </button>
                              </PopoverTrigger>
                              <PopoverContent className="w-auto p-0" align="start">
                                <Calendar
                                  mode="single"
                                  selected={form.warrantyExpiry ? new Date(form.warrantyExpiry) : undefined}
